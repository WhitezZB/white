 ...
 * @lucene.experimental
 */
public abstract class CustomBufferedIndexInput extends HermesInput {
  ...
  private void refill() throws IOException {
    ...
    if (buffer == null) {
      buffer = new byte[bufferSize];//store.takeBuffer(bufferSize);
...
//    store.putBuffer(buffer);
  ...
  public HermesInput slice(String sliceDescription, long offset, long length) throws IOException {
	  return new SlicedIndexInput(sliceDescription, this, offset, length,this.bufferSize);
  }
  
  private static final class SlicedIndexInput extends CustomBufferedIndexInput {
	  HermesInput base;
	    long fileOffset;
	    long length;
	    
	    SlicedIndexInput(String sliceDescription, HermesInput base, long offset, long length,int buffersize) {
   ...
   */
  protected int flushBuffer(HermesOutput out, long numBytes) throws IOException {
